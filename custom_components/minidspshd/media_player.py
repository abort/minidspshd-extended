...
class Volumio(MediaPlayerEntity, RestoreEntity):
    ...
    def _on_minidsp_update(self, data: MiniDSPState) -> None:
        ...
        s = "ON" if data.dirac else "OFF"
        self.make_dirac_preset_map(data.dirac)
        self._attr_sound_mode = f"Preset {int(data.preset) + 1} (Dirac: {s})"
        self._attr_is_volume_muted = data.mute
        self._state["mute"] = data.mute
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if self._power_switch is not None:
            ...
            if power_state is not None:
                self._update_power_state(power_state.state)
